...
const imageModal = ref<HTMLImageElement>()
...
useEventListener('click', async (e) => {
  ...
  imageModal.value = first as HTMLImageElement
})
...
<template>
  ...
  <Transition name="fade">
    <div v-if="imageModal" fixed top-0 left-0 right-0 bottom-0 z-500 backdrop-blur-7 @click="imageModal = undefined">
      <div absolute top-0 left-0 right-0 bottom-0 bg-black:30 z--1 />
      <img :src="imageModal.src" :alt="imageModal.alt" :class="imageModal.className" max-w-screen max-h-screen w-full h-full object-contain>
    </div>
  </Transition>
</template>
